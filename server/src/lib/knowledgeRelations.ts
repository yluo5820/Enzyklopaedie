...
export const isKnowledgeRelationEntityType = (value: unknown): value is KnowledgeRelationEntityType =>
  value === 'knowledge_item' || value === 'topic' || value === 'reference_entity';

const relationDetailSelect = `
  SELECT
    kr.*,
    COALESCE(source_item.title, source_topic.name, source_reference.title) AS fromEntityTitle,
    COALESCE(
      source_item.kind,
      CASE WHEN kr.fromEntityType = 'topic' THEN 'topic' END,
      source_reference.kind
    ) AS fromEntityKind,
    COALESCE(target_item.title, target_topic.name, target_reference.title) AS toEntityTitle,
    COALESCE(
      target_item.kind,
      CASE WHEN kr.toEntityType = 'topic' THEN 'topic' END,
